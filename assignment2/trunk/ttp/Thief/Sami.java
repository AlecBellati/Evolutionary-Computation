...
public class Sami {
  ...
  public void getSolution(TTPInstance ttp) {
    ...
    for (int i = 0; i < generations; i++) {
      ...
      // Ensure the knapsack is still valid
      if (knapsack.getCurrentWeight() > knapsack.getCapacity()) {
        repairKnapsack();
      }
  ...
  /**
   * Repairs the knapsack in case it has been overfilled.
   */
  public void repairKnapsack() {
    
    // Sort items in order to determine which ones to remove
    ArrayList<Item> allItems = sortByWeightedCost(knapsack.getItems());
    
    // Remove items from back of list as they are the least profitable
    int i = 1;
    while (knapsack.getCurrentWeight() > knapsack.getCapacity()) {
      knapsack.removeItem(allItems.get(allItems.size()-i));
      i++;
    }
  }
